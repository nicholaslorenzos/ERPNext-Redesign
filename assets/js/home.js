...
let username = 'Admin'; // Would come from your authentication system
let taskCompleted = [false, false, false]; // Track completion status of tasks

// Redesigned Reports & Masters Data with 4 categories
const categoriesData = [
  {
    title: 'Accounting',
    icon: 'FileText',
    items: ['Chart of Accounts', 'Company', 'Customer', 'Supplier']
  },
  {
    title: 'Stock',
    icon: 'Package',
    items: ['Item', 'Warehouse', 'Brand', 'Unit of Measure (UOM)', 'Stock Reconciliation']
  },
  {
    title: 'CRM',
    icon: 'Users',
    items: ['Lead', 'Customer Group', 'Territory']
  },
  {
    title: 'Data Import and Settings',
    icon: 'Settings',
    items: ['Import Data', 'Opening Invoice Creation Tool', 'Chart of Accounts Importer', 'Letter Head', 'Email Account']
  }
];
...
// Helper function to get an icon SVG or placeholder
function getIconSvg(iconName, cssClass = "w-5 h-5") {
  // Get the SVG HTML from dashboardIcons or use Circle as fallback
  const svgHtml = dashboardIcons[iconName] || dashboardIcons["Circle"];
...
// Function to render stat cards with color-coded trends
...
// Function to render activity items with square backgrounds
  ...
  return `
    <div class="flex items-start gap-4 p-4 hover:bg-gray-50 rounded-xl transition-all duration-200 group cursor-pointer border border-transparent hover:border-gray-200">
      <div class="p-2.5 rounded-lg shadow-sm min-w-[40px] min-h-[40px] flex items-center justify-center transition-colors" 
        ...
        <span style="color: ${styleSet.text};" class="group-hover:text-white transition-colors">${getIconSvg(icon, "w-5 h-5")}</span>
      </div>
      <div class="flex-1">
  ...
  return `
    ...
    >
      <div class="flex items-center gap-3">
        <span class="${isActive ? 'text-amber-300' : 'text-indigo-600'}">${getIconSvg(icon, "w-5 h-5")}</span>
...
// Function to render module items
function renderModuleItem(name) {
  return `
    <div class="flex items-center p-4 bg-white border border-gray-100 hover:border-indigo-200 rounded-xl transition-all shadow-sm hover:shadow-md group cursor-pointer">
      <div class="w-2 h-2 rounded-full bg-indigo-600 mr-3 group-hover:bg-amber-400 transition-colors"></div>
...
// Function to render quick access items
function renderQuickAccessItem(icon, name, category) {
  return `
    <div class="p-4 bg-white border border-gray-100 hover:border-indigo-300 rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer group">
      <div class="bg-indigo-50 text-indigo-600 p-3 rounded-xl mx-auto w-12 h-12 flex items-center justify-center mb-3 group-hover:bg-indigo-600 group-hover:text-white transition-all">
      ...
      <div class="text-center">
        ...
        <span class="text-xs text-gray-500 flex items-center justify-center mt-1">
          <span class="text-amber-500 mr-1">${getIconSvg("Tag", "w-3 h-3")}</span> ${category}
        </span>
      </div>
    </div>
  `;
...
function renderDashboard() {
  ...
  let dashboardHTML = `
    ...
    <div class="bg-gradient-to-r from-indigo-700 via-indigo-600 to-indigo-800 rounded-xl p-6 shadow-lg mb-8 relative overflow-hidden">
      ...
      <div class="relative z-10">
        ...
        <p class="text-indigo-100 text-lg mb-6">Here's what happening with your business today.</p>
        <div class="flex gap-3">
          <button 
            class="bg-white text-indigo-700 px-5 py-2 rounded-lg text-sm font-medium hover:bg-indigo-50 transition-colors shadow-md"
            onclick="scrollToSectionWithOffset('stats-section', -80)"
          >
            View Reports
          </button>
          <button 
            class="bg-indigo-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-indigo-500 transition-colors border border-indigo-500 shadow-md"
    ...
    <!-- Quick Actions Grid -->
    <div id="quick-actions-section" class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <!-- REDESIGNED: Reports & Masters - Modern Design -->
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-8 mt-8">
        ...
        <div class="relative z-10 flex items-center justify-between">
          ...
          <div class="relative w-64">
            ...
            <input
              id="module-search"
              type="text"
              class="block w-full pl-10 pr-3 py-2 border border-indigo-500 bg-indigo-600 bg-opacity-50 rounded-lg text-indigo-100 placeholder-indigo-300 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent"
      ...
      <!-- Main Content Area - Split into Categories and Details Panels -->
      <div class="flex flex-col md:flex-row min-h-[400px]">
        <!-- Left Panel - Categories -->
        <div class="w-full md:w-1/4 bg-indigo-50 p-4 border-r border-gray-100">
          <h3 class="font-medium text-indigo-800 mb-3 text-sm uppercase tracking-wider">Categories</h3>
          ...
          ` : `
            <div class="mb-6">
              <div class="flex items-center gap-3 mb-2">
                <span class="text-indigo-600">${getIconSvg(activeCategoryData.icon, "w-6 h-6")}</span>
      ...
      <div class="border-t border-gray-100 p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <button class="text-sm text-indigo-600 hover:text-indigo-700 font-medium flex items-center px-4 py-2 border border-indigo-100 rounded-lg hover:bg-indigo-50 transition-all">
    ...
    <!-- Stats Grid -->
    <div id="stats-section" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
    ...
    <!-- Enhanced Chart and Summary Section -->
      <div class="bg-white rounded-xl p-6 shadow-lg border border-gray-100 mt-8 overflow-hidden">
        <div class="flex items-center justify-between mb-8">
          <div class="flex items-center gap-3">
            <div class="bg-gradient-to-br from-indigo-600 to-indigo-700 p-3 rounded-xl shadow-md min-w-[44px] min-h-[44px] flex items-center justify-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2">
            <div class="chart-container rounded-xl overflow-hidden bg-white">
              <div class="h-full flex flex-col">
                ...
                <div class="p-4 flex-grow" id="revenue-chart" style="min-height: 32rem;">
                  <!-- Chart will be rendered here by charts.js -->
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl p-5 border border-gray-100 shadow-sm flex flex-col justify-between">
              ...
              <div class="flex items-center justify-between mb-6">
                <h4 class="text-gray-800 font-semibold flex items-center gap-2">
                  <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 p-2.5 rounded-lg min-w-[36px] min-h-[36px] flex items-center justify-center">
                    <span class="text-indigo-600">${getIconSvg("BarChart2", "w-4 h-4")}</span>
                  </div>
                  <span>Performance Metrics</span>
                </h4>
                <button class="text-sm text-indigo-600 hover:text-indigo-700 font-medium px-3 py-1.5 bg-white rounded-lg hover:bg-indigo-50 transition-all border border-indigo-100 shadow-sm">
                  View Report
                </button>
              </div>
                ...
                ${[
                  ...
                  <div class="group hover:bg-white hover:shadow-sm p-3 rounded-lg transition-all duration-300 cursor-pointer border border-transparent hover:border-gray-200">
                      ...
                      <div>
                        <div class="flex items-center gap-2">
                          <div class="p-2 rounded-lg bg-${metric.color}-50 group-hover:bg-${metric.color}-100 transition-colors min-w-[32px] min-h-[32px] flex items-center justify-center">
                            <span class="text-${metric.color}-600">${getIconSvg(metric.icon, "w-4 h-4")}</span>
                          </div>
                    ...
                    <div class="h-2 bg-gray-100 rounded-full overflow-hidden mt-3">
                      <div 
                        class="h-full bg-${metric.color}-500 rounded-full transform transition-all duration-500 group-hover:translate-x-1"
            ...
            <div class="mt-6 pt-6 border-t border-gray-100">
              <button class="w-full py-2.5 px-4 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-lg hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 text-sm font-medium shadow-sm hover:shadow">
                Update Metrics
              </button>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl p-6 shadow-lg border border-gray-100">
          <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
              <div class="bg-indigo-600 p-2.5 rounded-lg shadow-md min-w-[40px] min-h-[40px] flex items-center justify-center">
            ...
            ${[
              ...
              <div class="p-4 border border-gray-200 hover:border-indigo-300 rounded-xl transition-colors cursor-pointer group hover:shadow-sm">
                <div class="flex items-center mb-2">
                  <div class="task-checkbox cursor-pointer w-5 h-5 rounded border-2 border-indigo-300 mr-3 flex-shrink-0 flex items-center justify-center ${taskCompleted[item.index] ? 'bg-indigo-500 border-indigo-500' : ''}" data-index="${item.index}">
                    ${taskCompleted[item.index] ? `<span class="text-white">${getIconSvg("Check", "w-3 h-3")}</span>` : ''}
                  </div>
                  <h4 class="text-sm font-medium text-gray-800 group-hover:text-indigo-600 ${taskCompleted[item.index] ? 'line-through text-gray-400' : ''}">${item.task}</h4>
                </div>
  ...
  document.querySelectorAll('.time-range-button').forEach(button => {
    button.addEventListener('click', function() {
      ...
      document.querySelectorAll('.time-range-button').forEach(btn => {
        btn.classList.remove('bg-white', 'text-indigo-600', 'shadow-sm', 'border', 'border-gray-200');
        btn.classList.add('text-gray-600', 'hover:text-gray-900');
      });
      
      this.classList.remove('text-gray-600', 'hover:text-gray-900');
      this.classList.add('bg-white', 'text-indigo-600', 'shadow-sm', 'border', 'border-gray-200');
  ...
  // Add yellow outline focus effect to module search input
  const moduleSearch = document.getElementById('module-search');
  if (moduleSearch) {
    moduleSearch.addEventListener('focus', () => {
      moduleSearch.style.outline = 'none';
      moduleSearch.style.boxShadow = '0 0 0 2px #FCD34D'; // amber-300
  ...
  // Add task checkbox and box click functionality
  ...
  // Make entire task box clickable
  document.querySelectorAll('.task-checkbox').forEach(checkbox => {
